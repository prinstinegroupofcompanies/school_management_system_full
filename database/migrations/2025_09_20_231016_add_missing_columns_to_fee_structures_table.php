<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        // Skip this migration entirely - all columns already exist in the fee_structures table
        \Log::info('Skipping migration 2025_09_20_231016_add_missing_columns_to_fee_structures_table - all columns already exist');
        
        // Note: The amount column is intentionally kept for backward compatibility
        // Both amount and total_amount are now used in the FeeStructure model
        return;
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        // Add back the amount column
        Schema::table('fee_structures', function (Blueprint $table) {
            $table->decimal('amount', 10, 2)->after('class_id');
        });
        
        // Copy data from total_amount to amount
        DB::statement('UPDATE fee_structures SET amount = total_amount WHERE amount IS NULL');
        
        // Remove added columns
        Schema::table('fee_structures', function (Blueprint $table) {
            $table->dropColumn([
                'is_active',
                'status',
                'max_installments',
                'allow_installments',
                'late_fee_amount',
                'late_fee_percentage',
                'grace_period_days',
                'due_date',
                'final_amount',
                'discount_amount',
                'discount_percentage',
                'total_amount',
                'fee_type',
                'academic_year',
                'description',
                'name'
            ]);
        });
    }
};
