...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div class="bg-white shadow rounded-lg">
            <div class="px-6 py-6">
                <form action="{{ route('finance.fees.structures.update', $feeStructure) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="academic_year" name="academic_year" 
                                ...
                                <option value="2024-2025" {{ old('academic_year', $feeStructure->academic_year) === '2024-2025' ? 'selected' : '' }}>2024-2025</option>
                                <option value="2023-2024" {{ old('academic_year', $feeStructure->academic_year) === '2023-2024' ? 'selected' : '' }}>2023-2024</option>
                                <option value="2022-2023" {{ old('academic_year', $feeStructure->academic_year) === '2022-2023' ? 'selected' : '' }}>2022-2023</option>
                            </select>
                            @error('academic_year')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" 
                                  ...
                                  placeholder="Enter fee structure description">{{ old('description', $feeStructure->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div id="fee-items-container">
                            @if(isset($feeStructure->feeItems) && $feeStructure->feeItems->count() > 0)
                                @foreach($feeStructure->feeItems as $index => $item)
                                <div class="fee-item border border-gray-200 rounded-lg p-4 mb-4">
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div>
                                            ...
                                            <input type="text" name="fee_items[{{ $index }}][type]" value="{{ $item->type }}" 
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="fee_items[{{ $index }}][amount]" value="{{ $item->amount }}" step="0.01" 
                                                   class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500" 
                                                   placeholder="0.00" required>
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Due Date</label>
                                            <input type="date" name="fee_items[{{ $index }}][due_date]" value="{{ $item->due_date }}" 
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <textarea name="fee_items[{{ $index }}][description]" rows="2" 
                                                  class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500" 
...
@push('scripts')
<script>
let feeItemIndex = {{ isset($feeStructure->feeItems) ? $feeStructure->feeItems->count() : 1 }};
