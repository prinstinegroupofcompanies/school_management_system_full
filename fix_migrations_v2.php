<?php
/**
 * Fix Migration Dependencies Script V2
 * This will properly fix foreign key constraints in migration files
 */

echo "=== Fixing Migration Dependencies V2 ===\n\n";
...
foreach ($migrationFiles as $file) {
    ...
    if (file_exists($filePath)) {
        ...
        $content = preg_replace('/\$table->unsignedBigInteger;\s*\$table->unsignedBigInteger->nullable\(\);/', '$table->unsignedBigInteger(\'class_id\')->nullable();', $content);
        $content = preg_replace('/\$table->unsignedBigInteger;\s*\$table->unsignedBigInteger\(\);/', '$table->unsignedBigInteger(\'class_id\');', $content);
        
        // Remove foreign key constraints properly
        $content = preg_replace('/->constrained\([^)]+\)/', '', $content);
        $content = preg_replace('/->onDelete\([^)]+\)/', '', $content);
        $content = preg_replace('/->onUpdate\([^)]+\)/', '', $content);
        
        // Replace foreignId with unsignedBigInteger properly
        $content = preg_replace('/foreignId\(([^)]+)\)/', 'unsignedBigInteger($1)', $content);
